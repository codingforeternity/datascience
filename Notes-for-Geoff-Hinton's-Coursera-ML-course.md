...
* *Inference* in an infinite sigmoid belief net
  ...
  * The (inference) process that goes from h0 to v1 (up) is exactly the same as that (the generative process) which goes from h0 to v0 (down), which means that s_i^1 is an unbiased sample from p_i^0
