...
* Sampling-based Approaches - only useful at training time -- during inference (at test time) the full softmax still needs to be computed to obtain a normalised probability.
  ...
  * **Negative Sampling (NEG)** - the objective that has been popularised by Mikolov et al. (2013), can be seen as an **approximation to NCE** - earn high-quality word representations rather than achieving low perplexity
